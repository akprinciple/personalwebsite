...
if (isset($_POST['submit'])) {
	...
	$select = "SELECT * FROM education WHERE degree = '{$degree}' && institution = '{$institution}'";
	...
	elseif ($year == "Select Year") {
		$msg = "<div class='alert-danger text-center p-2 rounded'>Select Year</div>";
	}
	else{
	$sql = "INSERT INTO education (degree, institution, about, year, date) VALUES ('$degree', '$institution', '$about', '$year', '$date')";
...
<div class="row ml-0 mr-0">
	<div class="col-md-10">
	<h2 class="mt-3 text-light">Educational Qualification</h2>
...
<!----------------- Form for Adding Education Details ---------------------->
	<div  id="showcase" class="col-md-9 m-auto p-3 text-orange bg-white" style="display: none;">
	<form method="post" enctype="multipart/form-data">
	<h4 class="text-orange  font-weight-bold">Add Qualification</h4>
	<hr class="bg-orange">

	<div class="form-group">
	<label class="font-weight-bold">Degree :</label>
	<input type="text" required="" name="degree"  value="<?php echo $degree; ?>" placeholder="Enter Your Degree" class="form-control">

	<label class="font-weight-bold mt-3">Institution :</label>
	<input type="text" required="" name="institution" placeholder="Enter Your Institution" class="form-control" value="<?php echo $institution; ?>">
	
	<label class="font-weight-bold mt-3">About</label>
	<textarea name="about" id="textarea"><?php echo $about; ?></textarea>

	<label class="font-weight-bold mt-3">Year :</label>
	<select name="year" class="form-control">
		<option>Select Year</option>
		<?php 
		$date = date('Y');
		for ($date; $date >= 1960; $date--) { 
	...
	<button type="submit" name="submit" class="col-md-4 float-right darkorange p-2 border-0 mb-3 outline text-light btn">Submit</button>
	<div class="clearfix"></div>
	</form>
	</div>
	<h4 class="text-light">Manage Education Details</h4>
	<table class="table table-striped table-bordered text-center bg-white">
		...
		<tr>
			...
			<td><?php echo $row['institution']. " - ". $row['year']; ?></td>
			<td><?php echo substr($row['about'], 0, 50) ." ... " ; ?></td>
			<td><?php echo $row['date']; ?></td>
			<td>
			<span id="update<?php echo $row['id']; ?>" class="fas fa-pen pointer"></span>
			<span id="del<?php echo $row['id']; ?>" class="fas fa-times text-danger pointer"></span>
			</td>
		</tr>
		<?php } ?>
		</tbody>
...
<div class=" bg-dark w-100 text-light text-center position-absolute p-2" style="bottom: 0; ">Are you sure you want to <b class="text-danger"> DELETE</b> the report for  
	...
	permanently?
<a href="delete.php?del_education=<?php echo $user['id']; ?>"><button class="btn-success btn">Yes</button></a>
...
<div id="fetcher<?php echo $user['id']; ?>" class="w-100 position-absolute position-fixed" style="background-color: rgba(0,0,0,0.5); min-height: 100%; top: 0; z-index: 2; display: none;">
	...
	<h4 class="text-center font-weight-bold">Edit Qualification</h4>
	<hr>
	...
	<div class="form-group ">
 	<input type="text" required="required" name="degree" value="<?php echo $user['degree'] ?>" class="text-center form-control">
	</div>
	</div>
	...
	<div class="form-group">
 	<input type="text" required="required" name="institution" value="<?php echo $user['institution'] ?>" class="text-center form-control">
	</div>
	</div>
	...
	<label>About</label>
	<div class="form-group">
 	<textarea name="about" row="7" class="form-control"><?php echo $user['about'] ?></textarea>
	</div>
	</div>
	<div class="col-md-6">
	<label>Year</label>
	<div class="form-group">
		<select class="form-control" name="year">
			<option><?php echo $user['year'] ?></option>
			<?php 
		$date = date('Y');
		for ($date; $date >= 1960; $date--) { 
	...
	<div class="col-md-6">
	<button type="submit" name="ind_update<?php echo $id; ?>" style="vertical-align: bottom;" class="btn btn-success mt-4 w-100">update</button>
	<!-- <div class="clearfix"></div> -->
	</div>
	</div>
...
$year = mysqli_real_escape_string($connect, $_POST['year']);
$date = date('d/M/Y');
$ind_sql = "UPDATE education SET degree = '{$degree}', institution = '{$institution}', about = '{$about}', year = '{$year}', date = '{$date}' WHERE id = '{$user["id"]}'";
