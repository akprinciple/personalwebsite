...
<h4 class="text-light">Manage Work Experience</h4>
	<table class="table table-striped table-bordered text-center bg-white">
		...
		<tr>
			...
			<a href="editexperience.php?edit=<?php echo $row['id']; ?>&&position=<?php echo $row['position']; ?>" class="fas fa-pen-alt"></a>
...
?>
<div id="fetcher<?php echo $id ?>" class="w-100 position-absolute position-fixed" style="background-color: rgba(0,0,0,0.5); min-height: 100%; top: 0; z-index: 2; display: none;">
	<div class="col-md-10 m-auto p-3 shadow">
		<span id="clearer" class="fas fa-times text-danger float-right fa-2x"></span>
	...
	<form method="post" id="<?php echo $user['id']; ?>" enctype="multipart/form-data">
	<div class="row">
	<div class="col-md-6">
	...
	<button type="submit" name="ind_update<?php echo $id; ?>" class="btn btn-success mt-4 w-100">update</button>
	</div>
	<div class="clearfix"></div>
...
$roles = mysqli_real_escape_string($connect, $_POST['roles']);
$date = date('d/M/Y');
$ind_sql = "UPDATE experience SET position = '{$position}', company = '{$company}', roles = '{$roles}', date = '{$date}' WHERE id = '".$user["id"]."' ";
...
$(document).ready(function() {
  ...
  $("#fetcher<?php echo $id; ?>").toggle("slow");
  })
  $("#clearer").click(function(){
  $("#fetcher<?php echo $id; ?>").hide("slow"); 
})
})                     
</script>
